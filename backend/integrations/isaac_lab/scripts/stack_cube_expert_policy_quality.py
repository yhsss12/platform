"""Pure helpers for stack_cube_expert_policy (no Isaac import)."""
...
def min_jerk_progress(step: int, duration_steps: int) -> float:
    """Normalized min-jerk s in [0, 1] for smooth segment interpolation."""
    if duration_steps <= 0:
        return 1.0
    s = min(1.0, max(0.0, step / float(duration_steps)))
...
def interpolate_pose(
    ...
    *,
    ...
    t = min_jerk_progress(step, duration_steps)
...
class ExpertQualityTracker:
    ...
    _current_episode_max_rot_cmd: float = 0.0

    def begin_attempt(self) -> None:
        self.attempts += 1
        self._current_episode_steps = 0
        self._current_episode_max_arm_delta = 0.0
        self._current_episode_max_rot_cmd = 0.0
    ...
    def record_step_action(self, action: torch.Tensor, previous_action: Optional[torch.Tensor]) -> None:
        ...
        rot_cmd = float(torch.norm(action[3:6]).item()) if action.numel() >= 6 else 0.0
        self._current_episode_max_rot_cmd = max(self._current_episode_max_rot_cmd, rot_cmd)
    ...
    def finish_attempt_failure(
        ...
        *,
        ...
        failure_reason: str,
        cube_index: int,
        last_ee_pose: Optional[list[float]] = None,
        last_object_pose: Optional[list[float]] = None,
    ) -> None:
        ...
        entry: dict[str, Any] = {
            ...
            "maxArmActionDelta": self._current_episode_max_arm_delta,
            "maxRotCmd": self._current_episode_max_rot_cmd,
        }
        if last_ee_pose is not None:
            entry["lastEePose"] = last_ee_pose
        if last_object_pose is not None:
            entry["lastObjectPose"] = last_object_pose
