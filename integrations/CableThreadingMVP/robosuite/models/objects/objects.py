...
class MujocoObject(MujocoModel):
    ...
    def _get_object_properties(self):
        ...
        self._bodies = [e.get("name") for e in _elements.get("bodies", []) if e.get("name") is not None]
        self._joints = [e.get("name") for e in _elements.get("joints", []) if e.get("name") is not None]
        self._actuators = [e.get("name") for e in _elements.get("actuators", []) if e.get("name") is not None]
        self._sites = [e.get("name") for e in _elements.get("sites", []) if e.get("name") is not None]
        self._sensors = [e.get("name") for e in _elements.get("sensors", []) if e.get("name") is not None]
        self._contact_geoms = [e.get("name") for e in _elements.get("contact_geoms", []) if e.get("name") is not None]
        self._visual_geoms = [e.get("name") for e in _elements.get("visual_geoms", []) if e.get("name") is not None]
...
class MujocoXMLObject(MujocoObject, MujocoXML):
    ...
    def _get_object_subtree(self):
        ...
        def _should_keep(el):
            return int(el.get("group", "0")) in GEOMTYPE2GROUP[obj_type]

        # Loop through each of these pairs and modify them according to @elements arg
        for i, (parent, element) in enumerate(geom_pairs):
            # Composite template geoms are compile-time macros, not runtime geoms.
            # They must keep the schema expected by MuJoCo and should not be renamed
            # or duplicated like regular object geoms.
