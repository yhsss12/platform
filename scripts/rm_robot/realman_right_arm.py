...
# ===================== 手动配置参数 =====================
# 机械臂网络参数
ARM_IP = "192.168.1.18"          # 机械臂IP
ARM_PORT = 8080                  # 机械臂TCP端口
LOCAL_IP = "192.168.1.20"        # 本地IP
LOCAL_PORT = 8098                # 本地UDP端口
...
GET_SIX_FORCE = True
FORCE_COORDINATE = 0
SIX_FORCE_TOPIC = "/right/rm_driver/get_force_data_result"
SIX_FORCE_USE_TCP_QUERY = True
SIX_FORCE_TCP_MIN_PERIOD = 0.05
SIX_FORCE_PREFER_ZERO = False
...
# ROS2配置
ROS2_JOINT_TOPIC = "/gello/right_arm/joint_states"
...
class JointStateSubscriber(Node):
    def __init__(self):
        ...
        # 订阅ROS2关节话题
        self.slave_gripper_pub = self.create_publisher(Float32, '/right_gripper_cmd', 10)
        self.slave_rm_pub = self.create_publisher(JointState, '/right/joint_states', 10)
    ...
    #    # 提取关节位置（弧度）→ 转角度
    #    joint_pos_rad = msg.position[:ARM_AXIS]
    #    joint_pos_deg = np.array(joint_pos_rad) * 180 / np.pi
    #    ROS2_JOINT_ANGLES_DEG = [round(deg, 3) for deg in joint_pos_deg]
    #    logging.debug(f"订阅到ROS2话题角度: {ROS2_JOINT_ANGLES_DEG} °")
