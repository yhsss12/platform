...
function formatApiDatasetDataFormat(dataset: Dataset): string {
  if (isLerobotSidecarDataset(dataset)) return 'LeRobot';
...
function canOpenDatasetTrainingForOption(dataset: Dataset): boolean {
  if (isLerobotSidecarDataset(dataset) && dataset.pi0Ready === true) {
    return Boolean(dataset.lerobotPath || dataset.builtDatasetPath) && (dataset.episodeCount ?? 0) > 0;
  }
...
export function datasetToTrainingDatasetOption(dataset: Dataset): TrainingDatasetOption | null {
  if (!canOpenDatasetTrainingForOption(dataset)) return null;
  const isDualArm =
    dataset.sourceJobId.startsWith('dac_gen_') || dataset.taskTemplateId === 'dual_arm_cable_manipulation';
  const isIsaac = isIsaacBlockStackingDataset(dataset);
  const isNutAssembly =
  ...
  return {
    ...
    modelFormat: isIsaac ? 'Robomimic BC' : isDualArm ? 'BC (PyTorch)' : 'Robomimic BC',
    dataFormat: formatApiDatasetDataFormat(dataset),
    sampleCount: dataset.episodeCount,
    dataCount: dataset.dataCount ?? dataset.episodeCount,
    createdAt: dataset.createdAt,
    format: dataset.format,
    datasetFormat: dataset.datasetFormat ?? undefined,
    ...
    taskType: isIsaac
      ...
      : isDualArm
        ...
        : isNutAssembly
          ? 'nut_assembly'
          : dataset.taskType || 'cable_threading',
    taskEnv: isIsaac ? 'Isaac-Stack-Cube-Franka-IK-Rel-v0' : undefined,
    simulatorBackend: dataset.simulatorBackend ?? undefined,
    pi0Ready: dataset.pi0Ready ?? undefined,
    lerobotPath: dataset.lerobotPath ?? undefined,
    availableFormats: dataset.availableFormats ?? dataset.mainFormats ?? undefined,
  };
}
...
export function dataItemToTrainingDatasetOption(item: WorkspaceDataItem): TrainingDatasetOption | null {
  ...
  return {
    ...
    pi0Ready: item.pi0Ready,
    lerobotPath: item.lerobotPath ?? undefined,
    availableFormats: item.mainFormats,
  };
}
...
export function listMergedTrainingDatasetOptions(
  ...
  // Completed filesystem / DB rows are authoritative. Local mock-flow rows
  // may retain the provisional name and task type used before the backend job
  // returned, which previously made NutAssembly appear as cable threading in
  // the training picker even though the API row was correct.
  for (const option of [...fromApi, ...fromItems]) {
...
export function formatTrainingModelTypeLabel(
  modelType: string,
  trainingBackend?: string | null
): string {
  const value = (modelType ?? '').trim();
  if (isUnknownTrainingModel(value)) return 'Unknown';
  return formatTrainingRecipeLabel(trainingBackend, value);
}
...
/** 新建训练任务默认名称：数据集 · 模型 · 时间 */
export function buildDefaultTrainingTaskName(options: {
  datasetName: string;
  modelLabel: string;
  at?: Date;
}): string {
  const dataset = (options.datasetName || '训练数据集').trim();
  const model = (options.modelLabel || '训练').trim();
  const at = options.at ?? new Date();
  const pad = (n: number) => String(n).padStart(2, '0');
  const timeLabel = `${at.getFullYear()}/${pad(at.getMonth() + 1)}/${pad(at.getDate())} ${pad(at.getHours())}:${pad(at.getMinutes())}`;
  return `${dataset} · ${model} · ${timeLabel}`;
}

export function randomTrainingSeed(): number {
  return Math.floor(Math.random() * 2_147_483_648);
}

export function resolveTrainingSeedInput(draft: string, placeholder: number): number {
...
export function sanitizeSeedDraftInput(raw: string): string {
  const digits = raw.replace(/\D/g, '');
  if (!digits) return '';
  const parsed = Number(digits);
  if (!Number.isFinite(parsed)) return '';
  return String(Math.min(parsed, 2_147_483_647));
}
...
export function formatTrainingDatasetTitle(
  ...
  if (row.dataset === '—' || row.dataset.toLowerCase() === 'unknown') {
...
export function canEnableTrainingEvaluation(row: TrainingTaskRow): boolean {
  ...
  if (row.backendStatus === 'failed' || row.backendStatus === 'backend_unavailable') return false;
...
export interface TorchBcAdvancedParams {
  hidden_dims: string;
  weight_decay: number;
}
