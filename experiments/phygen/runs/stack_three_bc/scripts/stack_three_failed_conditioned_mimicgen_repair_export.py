#!/usr/bin/env python3
"""
Failed-demo conditioned MimicGen repair for stack_three.
...
DEFAULT_CONFIG = "/home/zyf/mimicgen/MimicGen_physics_refine/datasets/generated/stack_three_d0/demo/mg_config.json"
DEFAULT_SUCCESS_HDF5 = "/home/zyf/mimicgen/MimicGen_physics_refine/datasets/generated/stack_three_d0/demo/demo.hdf5"
DEFAULT_FAILED_HDF5 = "/home/zyf/mimicgen/MimicGen_physics_refine/datasets/generated/stack_three_d0/demo/demo_failed.hdf5"
...
def nearest_success_anchor_residual(pos: dict[str, np.ndarray]) -> dict[str, float] | None:
    ...
    for anchor in SUCCESS_ANCHORS:
        ...
        if not common:
            continue
        xy = np.array(
            [np.linalg.norm(np.asarray(pos[name])[:2] - np.asarray(anchor[name])[:2]) for name in common],
            dtype=np.float64,
        )
        z = np.array(
            [abs(float(np.asarray(pos[name])[2] - np.asarray(anchor[name])[2])) for name in common],
            dtype=np.float64,
        )
        full = np.array(
            [np.linalg.norm(np.asarray(pos[name]) - np.asarray(anchor[name])) for name in common],
            dtype=np.float64,
        )
        row = {
            "anchor_xy": float(xy.mean()),
            "anchor_z": float(z.mean()),
            "anchor_mean": float(full.mean()),
            "anchor_max": float(full.max()),
        }
        row["anchor_energy"] = row["anchor_xy"] / 0.035 + row["anchor_z"] / 0.025 + row["anchor_max"] / 0.090
...
def stack_metrics(pos: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    anchor = nearest_success_anchor_residual(pos)
...
def stack_obs_dicts(obs_list: list[dict[str, Any]]) -> dict[str, np.ndarray]:
    ...
    return {key: np.stack([np.asarray(obs[key]) for obs in obs_list], axis=0) for key in keys}
...
def merge_success_and_repaired_hdf5(
    ...
    with h5py.File(success_hdf5, "r") as success, h5py.File(repaired_hdf5, "r") as repaired, h5py.File(output_path, "w") as out:
        ...
        total = 0
        out_idx = 0
        for src in [success, repaired]:
            keys = sorted(src["data"].keys(), key=demo_sort_key)
            for key in keys:
                src["data"].copy(key, data_out, name=f"demo_{out_idx}")
                grp = data_out[f"demo_{out_idx}"]
                if src is repaired:
                    grp.attrs["is_repaired"] = True
                else:
                    grp.attrs["is_repaired"] = False
...
def main() -> None:
    ...
    out_dir = Path(args.output_dir or (
        ...
        f"stack_three_failed_conditioned_repair_{time.strftime('%Y%m%d_%H%M%S')}"
    ))
    ...
    num_success_anchors = set_success_anchors(args.success_hdf5)
    ...
    if repaired_success_hdf5:
        write_repaired_success_hdf5(repaired_success_hdf5, repaired_trajs, args.success_hdf5)
    if merged_hdf5:
        if not repaired_success_hdf5:
            repaired_success_hdf5 = str(out_dir / "demo_repaired_success.hdf5")
            write_repaired_success_hdf5(repaired_success_hdf5, repaired_trajs, args.success_hdf5)
        merge_success_and_repaired_hdf5(args.success_hdf5, repaired_success_hdf5, merged_hdf5)
    summary = {
