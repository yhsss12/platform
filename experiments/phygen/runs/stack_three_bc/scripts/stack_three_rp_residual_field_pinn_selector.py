#!/usr/bin/env python3
"""
Train a stack_three repair-parameter residual-field PINN from feedback rollouts
...
from stack_three_failed_conditioned_mimicgen_repair import (
    demo_sort_key,
    load_failed_contexts,
    make_candidate,
    set_success_anchors,
)
...
def train_model(
    ...
    if use_component_head:
        ...
        model = RepairParameterPINN(x.shape[1]).to(device)
    else:
        model = nn.Sequential(
    ...
    for epoch in range(epochs):
        ...
        loss_success_correction = torch.zeros((), dtype=torch.float32, device=device)
...
def main() -> None:
    ...
    summary = {
        ...
        "target_failed_hdf5": args.target_failed_hdf5,
        "target_success_hdf5": args.target_success_hdf5,
        "num_target_success_anchors": num_target_success_anchors,
