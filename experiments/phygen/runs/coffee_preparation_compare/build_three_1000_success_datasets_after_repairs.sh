...
log(){ echo "[$(date '+%F %T')] $*" | tee -a "$LOGDIR/build_three_1000_success_datasets.log"; }
count_demos(){ $PY - "$1" <<'PY'
import h5py, sys
...
log "waiting for TS and Shared repair summaries"
while [ ! -f "$RUN/ts_repair_export/summary.json" ] || [ ! -f "$RUN/shared_phygen_export/summary.json" ]; do sleep 120; done
...
G3=$((RHIGH - RLOW))
G4=$RLOW
if [ "$G2" -lt 0 ] || [ "$G3" -lt 0 ] || [ "$G4" -lt 0 ]; then
  echo "invalid counts C=$C RTS=$RTS RSH=$RSH G2=$G2 G3=$G3 G4=$G4" >&2
  exit 2
fi
...
run_gen(){
  ...
  local cfg=$GENROOT/configs/demo_src_coffee_preparation_D0_${label}_${n}.json
  local folder=$GENROOT/coffee_preparation_d0_${label}_${n}_seed${seed}_success
  make_gen_config "demo_src_coffee_preparation_D0_${label}_${n}" "$n" "$cfg"
  ...
  if [ ! -f "$folder/demo_src_coffee_preparation_task_D0/demo.hdf5" ]; then
    ...
    cd "$ROOT"
    $PY mimicgen/scripts/generate_dataset.py --config "$cfg" --source "$SOURCE" --folder "$folder" --seed "$seed" --auto-remove-exp 2>&1 | tee "$LOGDIR/generate_${label}.log"
  else
    log "reuse existing generation $label at $folder"
  fi
  echo "$folder/demo_src_coffee_preparation_task_D0/demo.hdf5"
}
G2_HDF5=$(run_gen gen2 "$G2" 17202)
G3_HDF5=$(run_gen gen3 "$G3" 17203)
G4_HDF5=$(run_gen gen4 "$G4" 17204)
...
C=int(C); RTS=int(RTS); RSH=int(RSH); G2=int(G2); G3=int(G3); G4=int(G4)
def keynum(k):
    try: return int(k.split('_')[-1])
    except Exception: return 10**12
def copy_first(src_path, data_out, start_idx, n, label, is_repaired):
    if n <= 0 or not src_path:
        return start_idx, 0
    copied=0
    with h5py.File(src_path,'r') as src:
        for key in sorted(src['data'].keys(), key=keynum)[:n]:
            src['data'].copy(key, data_out, name=f'demo_{start_idx}')
            g=data_out[f'demo_{start_idx}']
            g.attrs['subset_source']=label
            g.attrs['is_repaired']=bool(is_repaired)
...
def build(out_path, parts):
    ...
    with h5py.File(success,'r') as base, h5py.File(out_path,'w') as out:
        ...
        total_samples=0; idx=0; counts={}
        for path,n,label,is_rep in parts:
            idx0=idx
            idx,copied=copy_first(path, data, idx, n, label, is_rep)
            counts[label]=copied
        for k in data.keys():
            g=data[k]
            total_samples += int(g.attrs.get('num_samples', g['actions'].shape[0]))
        data.attrs['total']=total_samples
        out.attrs['num_demos']=len(data.keys())
        if len(data.keys()) != 1000:
            raise RuntimeError(f'{out_path} has {len(data.keys())} demos, expected 1000; counts={counts}')
    return counts
# high group: C + R_high + gen2
high_R = max(RTS, RSH)
low_R = min(RTS, RSH)
high_out=outdir/f'demo_{high_name}_1000_success.hdf5'
low_out=outdir/f'demo_{low_name}_1000_success.hdf5'
mg_out=outdir/'demo_mimicgen_1000_success.hdf5'
...
summary={
 'base_success_C':C,'ts_repaired_R':RTS,'shared_repaired_R':RSH,
 'high_repair_group':high_name,'low_repair_group':low_name,
 'gen2_count':G2,'gen3_count':G3,'gen4_count':G4,
 'datasets': {high_name:str(high_out), low_name:str(low_out), 'mimicgen':str(mg_out)},
 'counts': {high_name:high_counts, low_name:low_counts, 'mimicgen':mg_counts},
