"""V2-B5.4 lift-preserving transport rollout."""
...
from lift_v2b54_objective import compute_lift_preserving_score
from lift_v2b54_refiner import LiftV2B54Params, build_lift_v2b54_waypoints_from_hdf5
...
def apply_lift_v2b54_step_overlay(
    action: np.ndarray,
    step: int,
    ...
    *,
    ...
    from lift_v2b51_sim_search import apply_lift_v2b51_step_overlay

    b51 = LiftV2B51Params(**{k: getattr(params, k) for k in LiftV2B51Params.__dataclass_fields__})
    b51.lift_speed_scale = min(float(b51.lift_speed_scale), 0.22)
    out = apply_lift_v2b51_step_overlay(action, step, b51, phases, stage1_lift_delta=stage1_lift_delta)
    ...
    lift_hold_end = phases.get("lift_hold_transport_end", transport_start)
    weak_gate = float(phases.get("weak_lift_gate_m", params.weak_lift_before_transport_m))

    if grasp_idx - 8 <= step <= grasp_idx + int(params.pre_close_hold_steps):
        out[6] = min(out[6], -0.94 - float(params.gripper_close_depth_bonus))

    reclose_start = phases.get("pre_lift_reclose_start", lift_begin)
    reclose_end = phases.get("pre_lift_reclose_end", lift_begin)
    if reclose_start <= step <= reclose_end:
        out[6] = min(out[6], -0.96 - float(params.pre_lift_reclose_strength) * 0.12)

    squeeze_end = phases.get("post_micro_squeeze_end", stage1_end)
    if stage1_end <= step <= squeeze_end:
        out[6] = min(out[6], -0.97 - float(params.post_micro_lift_squeeze_strength) * 0.1)
        out[2] *= 1.05

    if lift_begin <= step <= stage2_end:
        out[2] *= 1.35 + float(params.lift_direction_bias_z) * 4.0
        out[2] *= max(0.25, float(params.lift_speed_scale))
        out[0] *= float(params.lateral_preload_scale)
        out[1] *= float(params.lateral_preload_scale)
        out[6] = min(out[6], -0.99)

    if transport_start <= step <= lift_hold_end:
        out[0] *= 0.15
        out[1] *= 0.15
        out[2] *= 1.2
        out[6] = min(out[6], -0.99)
...
def execute_lift_v2b54_rollout(
    hdf5_path: str,
    demo_key: str,
    label: str,
    params: LiftV2B54Params,
    *,
    rollout_kind: str = "lift_v2b54_cem",
    partial_lift_delta_thresh: float = PARTIAL_LIFT_DELTA_THRESH,
) -> dict[str, Any]:
    proxy, _orig, target_eef, gripper, phases = build_lift_v2b54_waypoints_from_hdf5(
    ...
    for step in range(length):
        ...
        action = compute_closed_loop_waypoint_action(
            ...
            grip[step],
            env_args,
            speed_scale=min(float(params.lift_speed_scale), 0.22),
        )
        ...
        nut_z_lift_so_far = 0.0
        if nut_z_trace and grasp_idx < len(nut_z_trace):
            nut_z_lift_so_far = max(nut_z_trace[grasp_idx:]) - nut_z_trace[grasp_idx]
        action = apply_lift_v2b54_step_overlay(
            action,
            step,
            params,
            phases,
    ...
    if grasp_step < len(nut_z_trace):
        nut_z_at_grasp = nut_z_trace[grasp_step]
    ...
    partial_lift = bool(contact_diag.partial_lift_success) or nut_lift_phase_delta >= partial_lift_delta_thresh
    ...
    result: dict[str, Any] = {
        ...
        "lift_v2b54_params": params.to_dict(),
        "training_eligible": False,
        "exclude_from_lift_success_training": True,
        "success_flag": success_flag,
        "partial_lift_success": partial_lift,
